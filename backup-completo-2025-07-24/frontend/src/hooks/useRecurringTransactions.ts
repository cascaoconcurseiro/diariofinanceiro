...
export const useRecurringTransactions = () => {
  ...
  const addRecurringTransaction = useCallback((
    ...
    // CORREÇÃO: Usar utilitário para calcular próxima data válida
    ...
    console.log(`➕ Adding recurring transaction with corrected start date: ${nextValidDate.toLocaleDateString()}`, newTransaction);
  ...
  const deleteRecurringTransaction = useCallback((id: string, deleteGeneratedTransactions: boolean = true): void => {
    console.log('🗑️ Deleting recurring transaction:', id, 'deleteGenerated:', deleteGeneratedTransactions);
    
    // CORREÇÃO: Remover o lançamento recorrente
    setRecurringTransactions(prev => prev.filter(t => t.id !== id));
    
    // CORREÇÃO: Se solicitado, também remover todos os lançamentos gerados por este recorrente
    if (deleteGeneratedTransactions) {
      // Esta função será chamada pelo componente pai que tem acesso ao sistema unificado
      console.log(`🧹 Requesting deletion of all transactions generated by recurring ${id}`);
    }
  }, []);
  ...
  const getNextExecutionDate = useCallback((transaction: RecurringTransaction): Date | null => {
    ...
    if (!isValidRecurringDate(startDate)) {
      // Se a data de início já passou, calcular a próxima
